...
async function fetchUsageData() {
  ...
  try {
    ...
    try {
      ...
      const routerResponse = await fetch('/api/router-status').catch(() => null);
      if (routerResponse && routerResponse.ok) {
        routerStatusResult = await routerResponse.json();
      }
    } catch (apiErr) {
      const staticResp = await fetch('/data/usage.example.json');
      if (!staticResp.ok) throw new Error('Could not load data from API or static file');
      const staticData = await staticResp.json();
      result = {
        success: true,
        data: staticData.usage || [],
...
async function refreshRouterStatusOnly() {
  ...
  try {
    ...
    if (!response.ok) throw new Error('Router status unavailable');
